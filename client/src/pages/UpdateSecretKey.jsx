...
const UpdateSecretKey = ({}) => {
  ...
  const handleUpdateSecretKey = async (e) => {
    ...
    try {
      ...
      if (window.ethereum) {
        ...
        const token = new ethers.Contract(tokenAddress, CarbonCreditTokenABI.abi, signer);
  ...
  return (
    <div className="flex flex-col items-center p-10 bg-red-500 text-white rounded-2xl shadow-lg w-full max-w-md mt-10">
      <div className="text-xl font-bold mb-4 text-center">Update Secret Key</div>
      <form
      ...
      >
        ...
        />
        <button type="submit" className="p-2 bg-yellow-500 text-white rounded w-full">
          Update Secret Key
        </button>
      </form>
    </div>
